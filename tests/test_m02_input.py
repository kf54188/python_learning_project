...
class M02Input(unittest.TestCase):
    ...
    def test_user_input(self):
        ...
        业务需求：通过控制台输入姓名，然后将输入的姓名返保存的变量是，骑过函数返回该变量
        测试步骤：
        ...
        result = ask_name() # 要测试的函数，获取用户输入的姓名，并返回该姓名

        sys.stdin=sys.__stdin__ # 恢复原始stdin
    ...
    def test_collect_user_data(self):
        """
            测试在控制台上输入多行文字
            业务需求：1. 在控制台上按提示要求输入姓名、年龄、城市多行文字，并将每一行文字，各自保存到对应的变量中，
                    2. 合并成一段文字，并返回格式化的字符串。
                    3. 即输入："张三"、"18"、"上海"，返回给调用者；张三, 18岁, 来自: 上海
        ...
        fake_input= StringIO("张三\n18\n上海\n")
        sys.stdin=fake_input
        result = collect_user_data() # 要测试的函数，收集用户输入的数据，并返回格式化的字符串
